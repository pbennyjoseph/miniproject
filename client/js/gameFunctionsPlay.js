...
Connector = new Array(Size);
for (i = 0; i < Size; i++) Connector[i] = new Array(Size);
for (i = 0; i < Size; i++) {
  for (j = 0; j < Size; j++) Connector[i][j] = new Array(4);
}
Upd = new Array(Size);
...
function ShowComputedValues() {
  ...
  for (i_index = 0; i_index < Size; i_index++) {
    for (j_index = 0; j_index < Size; j_index++)
      window.document.images[GameElements_images[i_index][j_index]].title =
        ...
        "\n" +
        Math.round(Connector[i_index][j_index][2]) +
        "\n" +
        Math.round(Connector[i_index][j_index][0]) +
        "|" +
        Math.round(Connector[i_index][j_index][1]) +
        "->" +
        Math.round(Connector[i_index][j_index][0] + Connector[i_index][j_index][1]) +
        "\n" +
        Math.round(Connector[i_index][j_index][3]) +
        "->" +
        Math.round(Connector[i_index][j_index][2] + Connector[i_index][j_index][3]) +
        "\n" +
        Math.round(
          Connector[i_index][j_index][0] +
            Connector[i_index][j_index][1] +
            Connector[i_index][j_index][2] +
            Connector[i_index][j_index][3]
        ) +
        "\n" +
        Math.round(
  ...
  for (i_index = 0; i_index < Size; i_index++) {
    for (j_index = 0; j_index < Size; j_index++) {
      if (Color[i_index][j_index] == 0) {
        ...
        if (theLevel > 2) {
          for (kk = 0; kk < 4; kk++) Mp -= Connector[i_index][j_index][kk];
        }
  ...
  for (i_index = 0; i_index < Size; i_index++) {
    for (j_index = 0; j_index < Size; j_index++) {
      for (kk = 0; kk < 4; kk++) {
        ...
        Connector[i_index][j_index][kk] = 0;
      }
    }
  }
  for (i_index = 0; i_index < Size; i_index++) {
...
function SetComputedValues(i_index, j_index, kk, cc, llevel) {
  ...
  if (llevel > 1) {
    Connector[i_index][j_index][kk] = N / 5;
    if (N >= 2 && N < 10) {
      Connector[i_index][j_index][kk] = bb + N - 2;
      M -= 32;
    }
    if (N < 2) {
      oo = 30000;
      ...
      if (oo <= M + 104) {
        Connector[i_index][j_index][kk] = bb - (oo - M) / 4;
        M -= 64;
      }
      M += oo;
      M /= 2;
  ...
  if (i_index > 0 && i_index < Size - 1 && j_index > 0 && j_index < Size - 1)
    Connector[i_index][j_index][kk] += ddb;
  else Connector[i_index][j_index][kk] -= 2;
  if ((i_index == 0 || i_index == Size - 1) && (j_index == 0 || j_index == Size - 1))
    Connector[i_index][j_index][kk] /= 2; // /=4
  if (Connector[i_index][j_index][kk] > 68) Connector[i_index][j_index][kk] = 68; //66
...
function UpdatePic(i_index, j_index) {
  window.document.images[GameElements_images[i_index][j_index]].src =
    Pic[1 + Color[i_index][j_index]].src;
}

function UpdateScreen() {
  for (i_index = 0; i_index < Size; i_index++) {
    for (j_index = 0; j_index < Size; j_index++)
      document.images[GameElements_images[i_index][j_index]].src =
        Pic[1 + Color[i_index][j_index]].src;
  }
  // if (MoveCount<10)
  //   window.document.OptionsForm.Moves.value=" "+eval(MoveCount)+" ";
